...
## Если PDF генерируется из HTML — чини источник ⭐

Если твой PDF собирается из HTML (дизайн-шаблон, генератор), **главная причина
искажений шрифтов и склеенных пробелов — конвертер HTML→PDF**. Старые движки
(напр. **wkhtmltopdf**) превращают веб-шрифты в **Type3** (глифы без имени) —
тогда любой PDF→DOCX конвертер (включая Adobe) угадывает шрифт по каждому глифу.

Решение — рендерить HTML **headless-браузером (Chrome)**: он встраивает шрифты
как именованный subset TrueType, текст остаётся чистым. Тогда весь конвейер вниз
работает без артефактов:

```bash
pip install -r requirements-html.txt
playwright install chromium            # один раз скачать браузер

python scripts/html_to_pdf.py design.html design.pdf   # чистый PDF (не Type3)
python convert.py design.pdf --engine adobe            # чистый DOCX
```

Рекомендуемый конвейер для HTML-исходников:
**HTML → `html_to_pdf.py` (Chrome) → `convert.py --engine adobe` → DOCX.**
