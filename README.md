...
## Если PDF генерируется из HTML — почини шрифт в источнике ⭐

Если твой PDF собирается из HTML через headless-Chrome (`Page.pdf()`), а текст
после конвертации всё равно искажается и слипается, причина почти наверняка —
**вариативный (variable) шрифт**.

**Корень:** Chrome печатает variable-шрифты в PDF как **Type3** (контуры глифов
без имени шрифта). Тогда любой PDF→DOCX конвертер (включая Adobe) вынужден
угадывать шрифт по каждому глифу → патчворк шрифтов и склеенные слова.

**Доказано** в этом проекте: один и тот же Manrope через Chrome `Page.pdf()`:
- как `@font-face` с `font-weight: 200 800` (variable) → **Type3** ❌
- как статические инстансы по весам → **Type0** (именованный subset) ✅

**Решение:** в `@font-face` использовать **статические инстансы** шрифта вместо
одного вариативного. Скрипт генерирует готовый CSS-блок:

```bash
pip install -r requirements-html.txt   # ставит fonttools + playwright

# 1) сделать @font-face со статическими весами из вариативного шрифта
python scripts/make_static_fontface.py "Manrope[wght].ttf" \
    --family Manrope --weights 400 500 600 700 800 -o fontface.css

# 2) вставить fontface.css в HTML-генератор вместо variable @font-face
# 3) отрендерить и сконвертировать
python scripts/html_to_pdf.py design.html design.pdf   # теперь Type0
python convert.py design.pdf --engine adobe            # чистый DOCX
```

Рекомендуемый конвейер для HTML-исходников:
**HTML (статические шрифты) → `html_to_pdf.py` (Chrome) → `convert.py --engine adobe` → DOCX.**

`scripts/html_to_pdf.py` рендерит HTML→PDF через Chrome с правильными ключами
(`prefer_css_page_size`, `print_background`, нулевые поля, `media=print`).
